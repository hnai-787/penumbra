...
This started as a coursework assignment (a rule-matching packet simulator)
and has since been rebuilt into **fwlint**, an offline static analyzer for
firewall rulesets. The original academic program is preserved unmodified
under [`archive/academic-original/`](archive/academic-original/) — nothing
about the original assignment was deleted.
...
firewall-rule-engine-cpp/
  ...
  examples/
    ...
    json/example.json
  archive/academic-original/   # the original coursework program, untouched
  data/, output/, screenshots/ # original coursework artifacts
  project.yaml
```
...
5. For the original coursework artifact, see
   `archive/academic-original/` and the "Original Results" section below.
...
1. Preserved the original coursework program unmodified under
   `archive/academic-original/` (see PROJECT_NOTES.md).
...
Preserved from the original coursework submission — see
`archive/academic-original/` for the untouched program and data:
...
- All rule/packet data (original coursework and the new examples/
